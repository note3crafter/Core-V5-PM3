...
use TheNote\core\tile\Observer as ObserverTile;
use TheNote\core\utils\Facing;

class Observer extends Solid implements IRedstone {
    use RedstoneTrait;
    ...
    public function place(Item $item, Block $blockReplace, Block $blockClicked, int $face, Vector3 $clickVector, Player $player = null) : bool {
        ...
        $nbt = ObserverTile::createNBT($this);
        $nbt->setInt("runtimeId", $runtimeId);

        Tile::createTile("Observer", $this->getLevel(), $nbt);
        return true;
    }
    ...
    public function getBlockEntity() : ObserverTile {
        $tile = $this->getLevel()->getTile($this);
        $observer = null;
        if($tile instanceof ObserverTile){
            $observer = $tile;
        }else{
            $observer = Tile::createTile("Observer", $this->getLevel(), ObserverTile ::createNBT($this));
        }
        return $observer;
    }
