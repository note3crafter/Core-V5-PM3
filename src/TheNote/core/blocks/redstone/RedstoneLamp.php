...
class RedstoneLamp extends Solid implements IRedstone {
    use RedstoneTrait;
    ...
    public function place(Item $item, Block $blockReplace, Block $blockClicked, int $face, Vector3 $clickVector, Player $player = null) : bool {
        if ($this->isBlockPowered($blockReplace)) {
            $this->getLevel()->setBlock($blockReplace, new RedstoneLampLit());
        } else {
    ...
    public function onRedstoneUpdate() : void {
        if ($this->isBlockPowered($this)) {
            $this->getLevel()->setBlock($this, new RedstoneLampLit());
        }
    }
}