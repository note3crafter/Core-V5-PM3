...
class RedstoneWire extends Flowable implements IRedstone {
    use RedstoneTrait;
    ...
    public function getWeakPower(int $face) : int {
        ...
        if ($this->getSide(Facing::opposite($face)) instanceof RedstoneDiode) {
            return $this->getDamage();
        }
        ...
        for ($i = 0; $i < count($direction); ++$i) {
            ...
            if (RedstoneUtils::isPowerSource($side)) {
                return 0;
            } elseif ($side instanceof RedstoneWire || $side instanceof RedstoneDiode) {
                return 0;
            }
        }
        return $this->getDamage();
