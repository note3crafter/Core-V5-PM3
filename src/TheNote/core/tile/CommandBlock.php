...
use TheNote\core\blocks\redstone\commandblock\CommandBlock as CBlock;
...
use TheNote\core\server\selector\CommandSelector;
use TheNote\core\inventory\CommandInventory;
...
class CommandBlock extends Spawnable implements InventoryHolder, Container, Nameable, CommandSender {
    use NameableTrait {
    ...
    public function dispatch() : void {
        ...
        if (!($block instanceof CommandBlockChain)) {
            return;
        }
    ...
    public function setCommandBlockMode(int $mode) : void {
        if ($this->commandBlockMode != $mode) {
            ...
            if ($mode == 0) {
                $this->level->setBlock($this, new CBlock($damage));
            } elseif ($mode == 1) {
                $this->level->setBlock($this, new CommandBlockRepeating($damage));
            } elseif ($mode == 2) {
                $this->level->setBlock($this, new CommandBlockChain($damage));
            }
